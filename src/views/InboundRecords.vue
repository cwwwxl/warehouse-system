<template>
  <div>
    <div class="page-header"><h2>入库记录</h2></div>
    <div class="card-box">
      <div class="filter-bar">
        <el-input v-model="search" placeholder="搜索物品名称/编码/集装箱号" clearable style="width:260px" prefix-icon="Search" />
      ...
      <el-table :data="filteredLogs" stripe>
        ...
        <el-table-column prop="qty" label="入库数量" width="90" align="right">
          <template #default="{ row }"><span style="color:#059669;font-weight:700">+{{ row.qty }}</span></template>
        </el-table-column>
        <el-table-column prop="operator" label="操作人" width="80" />
        <el-table-column prop="remark" label="备注" min-width="100" />
      </el-table>
      <div style="margin-top:12px;text-align:right;color:var(--text-secondary);font-size:13px">
        共 {{ filteredLogs.length }} 条记录，入库总量: <strong style="color:#059669">{{ filteredLogs.reduce((s, r) => s + r.qty, 0) }}</strong>
      </div>
    </div>
  </div>
</template>
...
const allInLogs = computed(() => getAllLogs().filter(l => l.type === 'in'))

const filteredLogs = computed(() => {
  let list = allInLogs.value
  if (search.value) {
    const q = search.value.toLowerCase()
    list = list.filter(r => r.name.includes(q) || r.code.toLowerCase().includes(q) || r.containerId.toLowerCase().includes(q))
  }
  if (dateRange.value) {
